import Link from "next/link";
import { formatPrice } from "@/config";

interface ShopCtaProps {
  href: string;
  label: string;
  priceFrom?: number;
  variant?: "primary" | "secondary" | "navy";
  className?: string;
}

export function ShopCta({
  href,
  label,
  priceFrom,
  variant = "primary",
  className = "",
}: ShopCtaProps) {
  const displayLabel =
    priceFrom !== undefined
      ? `${label} — From ${formatPrice(priceFrom)}`
      : label;

  const variants = {
    primary:
      "bg-brand-green text-white hover:bg-brand-green-light",
    secondary:
      "border-2 border-brand-navy bg-white text-brand-navy hover:bg-brand-navy hover:text-white",
    navy:
      "bg-white text-brand-navy hover:bg-brand-green hover:text-white",
  };

  return (
    <Link
      href={href}
      className={`inline-flex items-center justify-center gap-2 rounded-full px-9 py-4 text-sm font-black uppercase tracking-[0.18em] transition-colors ${variants[variant]} ${className}`}
    >
      {displayLabel}
      <svg
        className="h-4 w-4"
        fill="none"
        viewBox="0 0 24 24"
        strokeWidth="2.5"
        stroke="currentColor"
      >
        <path
          strokeLinecap="round"
          strokeLinejoin="round"
          d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3"
        />
      </svg>
    </Link>
  );
}
